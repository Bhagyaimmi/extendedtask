...
class TaskManagementAPITestCase(TestCase):
    def setUp(self):
        ...
        self.task1 = Task.objects.create(title="Task 1", description="Task 1 Description", priority=1, deadline="2024-06-01", assigned_to=self.user, project=self.project, status='open')
        self.task2 = Task.objects.create(title="Task 2", description="Task 2 Description", priority=2, deadline="2024-06-15", assigned_to=self.user, project=self.project, status='open')
    ...
    def test_task_status_update(self):
        ...
        response = self.client.patch(reverse('task-detail', kwargs={'pk': self.task1.pk}), data, format='json')
        self.assertEqual(response.status_code, 200)
